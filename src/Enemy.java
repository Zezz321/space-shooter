...
public class Enemy {
    ...
    public Enemy(int x, int y, Type type, int wave) {
        ...
        float waveScale = 1 + (wave - 1) * 0.12f;

        switch (type) {
            case BASIC:
                health = maxHealth = 1;
                speedY = Math.min(2 * waveScale, 6);
                speedX = RAND.nextFloat() * 2 - 1;
                shootCooldown = Math.max(40, 80 - wave * 2);
                primaryColor = new Color(255, 80, 80);
                accentColor  = new Color(255, 160, 80);
                break;
            case FAST:
                health = maxHealth = 1;
                speedY = Math.min(4 * waveScale, 9);
                ...
                break;
            case TANK:
                health = maxHealth = 2 + wave / 3;
                speedY = Math.min(1 * waveScale, 3.5f);
                speedX = RAND.nextFloat() - 0.5f;
                shootCooldown = Math.max(30, 60 - wave);
                primaryColor = new Color(180, 50, 200);
                accentColor  = new Color(220, 120, 255);
                break;
            case ZIGZAG:
                health = maxHealth = 1;
                speedY = Math.min(2.5f * waveScale, 7);
                speedX = 3 + wave * 0.2f;
                shootCooldown = 999;
            ...
            case SHOOTER:
                ...
                speedY = 0.3f;
                speedX = 1.5f;
                shootCooldown = Math.max(25, 55 - wave * 2);
    ...
    public void update() {
        ...
        if (type == Type.SHOOTER && y > 120) speedY = 0;
